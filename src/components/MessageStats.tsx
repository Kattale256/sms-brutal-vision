...
const MessageStats: React.FC<MessageStatsProps> = ({
  ...
  const financeData = [{
    name: 'Income',
    amount: financialData.filter(item => item.type === 'income').reduce((sum, item) => sum + item.amount, 0)
  }, {
    name: 'Expenses',
    amount: Math.abs(financialData.filter(item => item.type === 'expense').reduce((sum, item) => sum + item.amount, 0))
  }];
