...
const TransactionStats: React.FC<TransactionStatsProps> = ({ transactions }) => {
  ...
  // Get the start and end dates from transactions
  const transactionDates = useMemo(() => {
    const dates = transactions.map(t => new Date(t.timestamp));
    if (dates.length === 0) return { startDate: new Date(), endDate: new Date() };
  ...
  const transactionAmountsByDate = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      amountsByDate[date] += Math.abs(t.amount);
    });
  ...
  const maxAmount = useMemo(() => {
    return Math.max(...Object.values(transactionAmountsByDate));
  }, [transactionAmountsByDate]);

  // Generate css variable value for date highlighting
  const getDateHighlight = useCallback((date: Date) => {
    const dateStr = date.toISOString().split('T')[0];
    if (!transactionAmountsByDate[dateStr]) return undefined;
    
    const intensity = (transactionAmountsByDate[dateStr] / maxAmount) * 100;
    return {
      backgroundColor: `rgba(255, 82, 82, ${intensity / 100})`,
      color: intensity > 50 ? 'white' : undefined,
      fontWeight: intensity > 30 ? 'bold' : undefined
    };
  }, [transactionAmountsByDate, maxAmount]);
  ...
  const exportToPDF = () => {
    ...
    const startDateStr = format(transactionDates.startDate, 'yyyy-MM-dd');
    const endDateStr = format(transactionDates.endDate, 'yyyy-MM-dd');
    
    doc.setFontSize(16);
    doc.text("Extracted Insights From Your Financial Transactions", 20, 20);
    doc.setFontSize(14);
    doc.text(`from ${startDateStr} to ${endDateStr}`, 20, 30);
    
    // Disclaimer
    doc.setFontSize(8);
    doc.text("Disclaimer: Results are AI generated by Firm D1 LDC Research Project", 20, 35);
    
    // Transaction Summary
    doc.setFontSize(14);
    doc.text("Transaction Summary", 20, 45);
    ...
    doc.text("Financial Overview", 20, finalY1 + 15);
    ...
    doc.text("Transaction Recipients", 20, finalY2 + 15);
    ...
    doc.text("Extracted By Firm D1 Research Project on E-Payment Message Notification Analysis.", 20, finalY3 + 20);
    doc.text("(c) 2025 FIRM D1, LDC KAMPALA", 20, finalY3 + 30);
    
    doc.save("transaction-insights.pdf");
  };
